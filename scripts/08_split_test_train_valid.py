...
if __name__ == '__main__':
	...
	input_titles = load_titles(input1_path, '.json')
	output_titles = load_titles(input2_path, '.bap_mgc')


	titles = list(set(input_titles).intersection(output_titles))
	...
	if test_split_index < len(titles):	
		test_titles = titles[:test_split_index]
	else:
		test_titles = titles
		print('Not enough titles!')

	titles = [title for title in titles if title not in test_titles]
	random.shuffle(titles)

	valid_ratio = 0.05
	valid_split_index = math.ceil(len(titles)*valid_ratio)

	if valid_split_index < len(titles):	
		valid_titles = titles[:valid_split_index]
	else:
		valid_titles = titles
		print('Not enough titles!')
